...
@pytest.mark.parametrize("method, endpoint, endpoint_args", [
    ("GET", "csvbase.get_table", {"username": "test", "table_name": "test"}),
    ("POST", "csvbase.create_row", {"username": "test", "table_name": "test"}),
    ("PUT", "csvbase.update_row", {"username": "test", "table_name": "test", "row_id": 1}),
    ("GET", "csvbase.get_row", {"username": "test", "table_name": "test", "row_id": 1}),
    ("DELETE", "csvbase.delete_row", {"username": "test", "table_name": "test", "row_id": 1}),
...
def test_cors__allowed_urls(client, method, endpoint, endpoint_args):
    ...
    access_control_headers = {k: v for k,v in resp.headers.items() if k.startswith("Access-Control-")}
